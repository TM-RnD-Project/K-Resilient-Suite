...
use crate::secp256k1::big;
use crate::secp256k1::big::BIG;
use crate::secp256k1::fp;
use crate::secp256k1::fp::FP;
use crate::secp256k1::dbig::DBIG;
use crate::secp256k1::ecp;
use crate::secp256k1::ecp::ECP;
use crate::secp256k1::rom;
use crate::rand::RAND;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Sigsecp256k1 no secp256k1 collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Sigsecp256k1",ph,cl,&mut domain);
