...
use crate::anssi::fp;
use crate::anssi::fp::FP;
use crate::anssi::dbig::DBIG;
use crate::anssi::ecp;
use crate::anssi::ecp::ECP;
use crate::anssi::rom;
use crate::rand::RAND;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Siganssi no anssi collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Siganssi",ph,cl,&mut domain);
