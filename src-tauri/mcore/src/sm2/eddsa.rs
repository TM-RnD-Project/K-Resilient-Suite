...
use crate::sm2::big;
use crate::sm2::big::BIG;
use crate::sm2::fp;
use crate::sm2::fp::FP;
use crate::sm2::dbig::DBIG;
use crate::sm2::ecp;
use crate::sm2::ecp::ECP;
use crate::sm2::rom;
use crate::rand::RAND;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Sigsm2 no sm2 collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Sigsm2",ph,cl,&mut domain);
