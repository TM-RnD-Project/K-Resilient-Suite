...
use crate::nums512w::big;
use crate::nums512w::big::BIG;
use crate::nums512w::fp;
use crate::nums512w::fp::FP;
use crate::nums512w::dbig::DBIG;
use crate::nums512w::ecp;
use crate::nums512w::ecp::ECP;
use crate::nums512w::rom;
use crate::rand::RAND;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Signums512w no nums512w collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Signums512w",ph,cl,&mut domain);
