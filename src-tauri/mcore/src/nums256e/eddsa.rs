...
use crate::nums256e::big;
use crate::nums256e::big::BIG;
use crate::nums256e::fp;
use crate::nums256e::fp::FP;
use crate::nums256e::dbig::DBIG;
use crate::nums256e::ecp;
use crate::nums256e::ecp::ECP;
use crate::nums256e::rom;
use crate::rand::RAND;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Signums256e no nums256e collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Signums256e",ph,cl,&mut domain);
