...
use crate::nums256w::big;
use crate::nums256w::big::BIG;
use crate::nums256w::fp;
use crate::nums256w::fp::FP;
use crate::nums256w::dbig::DBIG;
use crate::nums256w::ecp;
use crate::nums256w::ecp::ECP;
use crate::nums256w::rom;
use crate::rand::RAND;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Signums256w no nums256w collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Signums256w",ph,cl,&mut domain);
