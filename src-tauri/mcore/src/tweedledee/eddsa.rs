...
use crate::tweedledee::fp;
use crate::tweedledee::fp::FP;
use crate::tweedledee::dbig::DBIG;
use crate::tweedledee::ecp;
use crate::tweedledee::ecp::ECP;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Sigtweedledee no tweedledee collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Sigtweedledee",ph,cl,&mut domain);
