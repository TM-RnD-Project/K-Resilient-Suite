...
use crate::nist256::big;
use crate::nist256::big::BIG;
use crate::nist256::fp;
use crate::nist256::fp::FP;
use crate::nist256::dbig::DBIG;
use crate::nist256::ecp;
use crate::nist256::ecp::ECP;
use crate::nist256::rom;
use crate::rand::RAND;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Signist256 no nist256 collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Signist256",ph,cl,&mut domain);
