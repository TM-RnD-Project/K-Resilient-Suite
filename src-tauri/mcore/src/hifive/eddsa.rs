...
use crate::hifive::big;
use crate::hifive::big::BIG;
use crate::hifive::fp;
use crate::hifive::fp::FP;
use crate::hifive::dbig::DBIG;
use crate::hifive::ecp;
use crate::hifive::ecp::ECP;
use crate::hifive::rom;
use crate::rand::RAND;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Sighifive no hifive collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Sighifive",ph,cl,&mut domain);
