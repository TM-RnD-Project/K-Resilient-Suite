...
use crate::c13318::fp;
use crate::c13318::fp::FP;
use crate::c13318::dbig::DBIG;
use crate::c13318::ecp;
use crate::c13318::ecp::ECP;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Sigc13318 no c13318 collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Sigc13318",ph,cl,&mut domain);
