...
use crate::mdc::big;
use crate::mdc::big::BIG;
use crate::mdc::fp;
use crate::mdc::fp::FP;
use crate::mdc::dbig::DBIG;
use crate::mdc::ecp;
use crate::mdc::ecp::ECP;
use crate::mdc::rom;
use crate::rand::RAND;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Sigmdc no mdc collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Sigmdc",ph,cl,&mut domain);
