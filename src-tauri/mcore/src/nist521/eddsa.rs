...
use crate::nist521::big;
use crate::nist521::big::BIG;
use crate::nist521::fp;
use crate::nist521::fp::FP;
use crate::nist521::dbig::DBIG;
use crate::nist521::ecp;
use crate::nist521::ecp::ECP;
use crate::nist521::rom;
use crate::rand::RAND;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Signist521 no nist521 collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Signist521",ph,cl,&mut domain);
