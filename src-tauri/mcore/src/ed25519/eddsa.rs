...
use crate::ed25519::fp;
use crate::ed25519::fp::FP;
use crate::ed25519::dbig::DBIG;
use crate::ed25519::ecp;
use crate::ed25519::ecp::ECP;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("SigEd25519 no Ed25519 collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("SigEd25519",ph,cl,&mut domain);
