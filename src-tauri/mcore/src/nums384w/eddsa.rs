...
use crate::nums384w::big;
use crate::nums384w::big::BIG;
use crate::nums384w::fp;
use crate::nums384w::fp::FP;
use crate::nums384w::dbig::DBIG;
use crate::nums384w::ecp;
use crate::nums384w::ecp::ECP;
use crate::nums384w::rom;
use crate::rand::RAND;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Signums384w no nums384w collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Signums384w",ph,cl,&mut domain);
