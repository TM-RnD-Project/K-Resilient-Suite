...
use crate::nist384::fp;
use crate::nist384::fp::FP;
use crate::nist384::dbig::DBIG;
use crate::nist384::ecp;
use crate::nist384::ecp::ECP;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Signist384 no nist384 collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Signist384",ph,cl,&mut domain);
