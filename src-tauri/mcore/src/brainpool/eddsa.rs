...
use crate::brainpool::big;
use crate::brainpool::big::BIG;
use crate::brainpool::fp;
use crate::brainpool::fp::FP;
use crate::brainpool::dbig::DBIG;
use crate::brainpool::ecp;
use crate::brainpool::ecp::ECP;
use crate::brainpool::rom;
use crate::rand::RAND;
...
fn h2(ph: bool,ctx: Option<&[u8]>,r: &[u8],q: &[u8],m: &[u8]) -> DBIG {
    ...
    if ecp::AESKEY<=16 { // Ed25519??
        ...
        if ph || cl>0 {                   // if not prehash and no context, omit dom2()
            let dl=dom("Sigbrainpool no brainpool collisions",ph,cl,&mut domain);
            for i in 0..dl {
                sh.process(domain[i]);
            }
            if let Some(sctx) = ctx {
        ...
        let dl=dom("Sigbrainpool",ph,cl,&mut domain);
